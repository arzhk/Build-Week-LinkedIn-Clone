    ...
    getInfo = async () => {
        try{
            ...
            if(res.ok){
                ...
                console.log("this is my info", this.state.myInfo)
            }else{
    ...
    render() {
        ...
        return (
            <div className="invitations p-0 border mt-5">
                {/* {Array.from({ length: 1 }, (_, i) => i + 1).map((n) => <InvitationsLoader key={n}/>)} */}
                    <div className="top d-flex justify-content-between pl-3 py-2 brdr-bottom">
                        <h5>Invitations</h5>
                        <button className="ignore d-flex justify-content-center align-items-center" style={{width: "6rem"}}>See all 5</button>
                    </div>
                    
                    <div className="info">
                        {this.state.myInfo.length > 0 ? myInfo.splice(0, (this.state.myShow ? 3 : 6)).map(personIMayKnow => (
                            <div className="d-flex justify-content-between brdr-bottom py-2 my-2">
                                <div  className="d-flex myLink">
                                    <img className="image mx-3" src={personIMayKnow.image} alt="user-img" 
                                    id="profile-img" />
                                ...
                                <div className="d-flex">
                                    ...
                                    <button id="accept" className="d-flex justify-content-center align-items-center mr-3">Accept</button>
                                </div>
                            </div>
                        )) : Array.from({ length: 3 }, (_, i) => i + 1).map((n) => <InvitationsLoader key={n}/>)}
                    </div>
